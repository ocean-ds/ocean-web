...
O CardGroup é um componente usado para agrupar informações relacionadas em um layout organizado. Ele suporta diferentes variantes (minimal e header) e pode incluir título, subtítulo, contador e uma ação clicável.
...
```tsx
import type { ICardGroupProps } from '@useblu/ocean-react';
...
- **Minimal**: Exibe título, subtítulo, contador, conteúdo personalizado e ação
- **Header**: Exibe apenas título, subtítulo e contador (sem conteúdo ou ação)
...
>
  <div style={{ padding: '16px' }}>
    <p style={{ margin: 0 }}>
      Você tem 15 notificações não lidas. Clique em "Marcar como lidas" para
      visualizá-las.
    </p>
  </div>
</CardGroup>
