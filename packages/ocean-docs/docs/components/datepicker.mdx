...
Quando um usuário tenta selecionar um dia bloqueado, você pode exibir uma mensagem explicativa através da prop `disabledDaysMessage`. O tooltip aparece automaticamente ao clicar em um dia desabilitado e desaparece após 4 segundos.
...
O tooltip é posicionado entre o cabeçalho do mês e a grade de dias, com uma seta apontando para baixo, indicando visualmente a relação com os dias bloqueados no calendário.
\n**_ End Patch_**fragment**_ codeassistant to=functions.read_lints_instruction_REMOVEIALOG_TYPING_ComCallableACTERREADME_WRITE_DEBUG_DESTNOTE_COMPLETEROUND_SIMPLE_PURPOSE-LAST_MESSAGE_SIMPLE
COOKSIMUL_DESCRIPTIONcompletion_invalid_**GillDevelop md\*\*\*
...
| `disabledDaysMessage` | `string`                                              | -       | Mensagem exibida em tooltip ao tentar selecionar um dia bloqueado |
