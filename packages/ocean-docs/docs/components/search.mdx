...
| `value`        | `string`                                     | -       | Valor controlado do campo (modo controlado)  |
| `defaultValue` | `string`                                     | -       | Valor inicial do campo (modo não controlado) |
| `onChange`     | `(e: ChangeEvent<HTMLInputElement>) => void` | -       | Callback chamado quando o valor muda         |
| `id`           | `string`                                     | -       | Identificador único para acessibilidade      |

**Nota:** O componente Search aceita todas as props nativas de um elemento `<input>` HTML. O botão X para limpar o campo aparece automaticamente quando há texto digitado.
...
### Busca com valor inicial (modo não controlado)
